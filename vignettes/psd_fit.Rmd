...
We analyze the structure diagrams of the above four algorithms for TGP and HGDP.
...
```{r eval=FALSE}
result_HGDP_vi <- psd_fit_vi(data_HGDP, 7, 1e-5, 800)
```
...
We fit different hyperparameters K respectively, and select the K that maximizes the log-maximum likelihood function. We conduct experiments on TPG and HGDP datasets and different algorithms separately.
